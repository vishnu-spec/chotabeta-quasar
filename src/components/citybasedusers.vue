...
<div class="q-pa-md">
  ...
  <tbody style=" text-align:left;">
    <tr v-for="s in data" :key="data.index" style="border-bottom:1px solid #ccc">
      ...
      <td :data-th="s.address">{{s.address}}</td>
      <td :data-th="s.total_count">{{s.total_count}}</td>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'address', align: 'center', label: 'City', field: 'address', sortable: true},
        { name: 'total_count', align: 'center', label: 'Totalcount', field: 'total_count', sortable: true},
        { name: 'today_count', align: 'center', label: 'Todaycount', field: 'today_count', sortable: true},
      ],
      data:'',
    }
  },
...
this.gettabledata();
      ...
      headers: {
       ...
       axios.get('https://chotabeta.app/dev/testenv/api/rep/city-based-count',config)
      .then(function (response) {
        // handle success
        vm.loading = false;
        console.log(response,"getstateeee");
        var tot_count = response.data.total_data;

        var tdy_count = response.data.today_count;
      ...
      tdy_count.forEach(ele => {
        tot_count.forEach(emt => {
          ...
          if (emt.address == ele.address) {
           emt.today_count = ele.today_count;
          }
          // else{
          //     emt.today_count = 0;
          // }
