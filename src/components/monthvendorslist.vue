...
<div class="q-pa-md">
  ...
  <tbody style=" text-align:left;">
    <tr v-for="s in data" :key="data.index" style="border-bottom:1px solid #ccc">
      ...
      <td :data-th="s.Yesterday_orders">{{s.Yesterday_orders}}</td>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'Yesterday_orders', align: 'center', label: 'Yesterday orders', field: 'Yesterday_orders', sortable: true},
...
gettabledata(){
            var vm = this;
            vm.loading = true;
            console.log(vm.from_date,"asdfghj",vm.to_date);
      ...
      headers: {
       ...
       axios.get('https://chotabeta.app/dev/testenv/api/month-vendors-list',
           {
        ...
        // handle success
        // vm.loading = false;
        console.log(response,"getstateeee12323432");
        var tot_count = response.data.Yesterday_orders;

        var tdy_count = response.data.Month_orders;
